...
//    GLUT g = new GLUT();
    ...
    Timer timer2 = new Timer(3000, (e) -> {
        ...
        bul2.add(new bullet(enm.direction, enm.x, enm.y));
...
//        g.glutBitmapString(8, Integer.toString((int) (Timer--)/60));
    ...
    private void darwEnemy(GL gl) {
        Shared.DrawSprite(gl, enm.x, enm.y, animationenemy, textures, enm.size);

    }

    private void handleEnemyMove() {

        if (enm.direction == Directions.up) {
            if (enm.y - enm.step < 0) {
                return;
            }
            for (DrowBlocks b : blocks) {
        ...
        if (enm.direction == Directions.down) {
        ...
        if (enm.direction == Directions.left) {
            if (enm.x - enm.step < 0) {
                return;
            }
        ...
        if (enm.direction == Directions.right) {
